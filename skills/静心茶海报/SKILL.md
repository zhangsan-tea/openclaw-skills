...
- 结论：**中文 ≤6 行可直接做**（实测余量 39–133px）；中文 7 行临界；**中文 ≥8 行需精简或局部并行**

### ⭐⭐ 分行优先，并行为辅（2026-09-14 定稿原则，优先级最高）

**默认按语义分行，不要主动合并。** 短句各占一行是原卡自带的**呼吸节奏**，读起来更透气、
更有停顿感——**这是想要的效果，不是浪费空间**。

**按内容长短两套处理**：

| 情形 | 做法 |
|---|---|
| **内容短**（中文 ≤6 行） | **保持原有分行，一个字都不要并。** 实测光与暗卡 3 行、余量 133px，正是理想状态 |
| **内容长**（中文 ≥7 行，放不下） | 先试"能不能容纳"；确实放不下才动用并句。**只并在最适合的那几处**，不要全卡一次并光 |

⚠️ **「短句并一行」是应急手段，不是默认手法**（用户原话："这并不是一个强行的要求，
是一个方法的提示…能分行也尽量分行，不用把所有短句全合并到一行"）。

并句做法：把语义同组的短句用空格并入同一行，一行上限约 **16–17 字**（16.5px + 2.5px 字距下 323px 宽）。
并用在"语义同组且转折不在此处"的位置；**语义转折处仍保留换行**。

**冲突时的优先级：分行节奏 > 留白余量。** 宁可余量压到 24px，也不要为了宽松而把句子并成一行
（余量 <20px 才视为偏挤，此时改为精简修饰句、而不是继续并句）。

> 2026-09-14 修正：第一版把 14 张里大部分短句都并了，被用户否决。
> 第二轮把卡2/3/4/6/7/8 **整体还原**回原中文卡行数，卡9–14 在余量允许范围内尽量拆，
> 最终余量 24–133px、全部无折行。
