...
- 结论：**中文 ≤6 行可做**（实测余量 39–137px）；中文 7 行已临界；**中文 ≥8 行放不下**

### ⭐ 短句并一行（2026-09-14，用户主动提出）
中文金句里常有一串**很短的句子各占一行**（如「不需要困惑／不需要评论／不需要贴任何标签」），
每行只有 3–13 字却吃满 31.4px 行高，是**最浪费空间的分行方式**。
**做法：把语义同组的短句用空格并入同一行**，一行上限约 **16–17 字**（16.5px + 2.5px 字距下 323px 宽）。
例：`恐惧是最强烈的情感／而那份信任／可以覆盖它／消融它／让恐惧消失`（5 行）
→ `恐惧是最强烈的情感／而那份信任 可以覆盖它／消融它 让恐惧消失`（3 行），多出 63px 留白。

**这条同时是"精简纳入"的主力手段**：原判"中文 ≥8 行放不下"的卡
（共时性 8 行、我们都一样 8 行、个人真相 7 行、分别 11 行、合理化 12 行、视角态度意图 15 行），
先用短句并一行、再删可有可无的修饰句，**压到 3–5 行后全部纳入**，实测余量 44–107px。
2026-09-14 已把 14 张全部做成双语版（原中文卡共 14 张）。
**合并时遵守原卡的呼吸节奏**：语义转折处仍保留换行，不把整段挤成一坨。
