...
description: Build a clean baseline trunk for archival raw materials before
  knowledge ingestion. Use this skill when repeatedly running non-Wiki baseline
  governance on a material repository, including archive extraction, cross-batch
  dedup, sensitive-text candidate scan, binary redaction, generic filename
  queueing, provenance (year×inspection) extraction, and downloadable canonical
  manifest output.
...
### Known ERROR patterns in binary redaction (from 1079-file production run)

When running binary redaction at scale, expect these ERROR categories — most are safely ignorable:

| ERROR type | Typical message | Count (sample) | Action |
|---|---|---|---|
| `__MACOSX` metadata | `PackageNotFoundError` on `._*.docx` | ~15% of errors | **Ignore** — macOS resource forks, not real documents |
| Zero-byte placeholder | `PackageNotFoundError` or `BadZipFile` on size=0 files | ~5% | **Ignore** — empty template placeholders |
| Office temp lock | `~$filename.docx` | ~2% | **Ignore** — Office edit lock files |
| `.xls` old format | `InvalidFileException: openpyxl does not support .xls` | ~17% | **Fix**: use `xlrd` or LibreOffice `--convert-to xlsx` headless, then re-run |
| openpyxl TypeError (Fill) | `TypeError: expected <class 'openpyxl.styles.fills.Fill'>` | ~17% | **Fix**: reopen with `load_workbook(path, read_only=True)` (read_only avoids style parsing); or use `pandas.read_excel(engine='openpyxl')` with `read_only=True` |
| Real file corruption | `BadZipFile: File is not a zip file` on non-zero-size files | ~2% | **Investigate** — file may be genuinely corrupted |
| Encoding-mangled path | `PackageNotFoundError` on non-MACOSX, non-zero files | ~2% | **Investigate** — likely GBK/mojibake filename; try `os.fsencode()/fsdecode()` |

**Production lesson**: A 1079-file queue typically resolves as ~67% no_pii + ~13% skip_empty + ~9% skip_pdf_image + ~8% redacted_copy + ~3% ERROR. The "remaining" count is often zero — the script processes the entire queue in one pass; only ERROR and SKIP_pdf_image files need follow-up.
...
- **Historical policy drift**: never infer the current governance state solely from an old baseline report. Verify the live repository, `.gitignore`, output directories, and current project rules first. A past P0 run may have used copy-based redaction, while the effective policy has later changed to in-place redaction with archival materials excluded from the remote repository.
- **Nested recovery sources**: before removing any `_P0_clean_*` or `_治理归档` directory, scan the whole target tree for nested copies. A nested `_P0_clean_*` can be the only surviving source for canonical files accidentally removed from the main tree. Produce and verify an exact missing-file/recovery manifest before any cleanup.
- **Year-label validation**: a directory year is a useful candidate, not proof of the inspection year. Treat source-regulation dates and attached historical materials as weaker evidence; do not report an inference "accuracy" without manually confirmed gold labels. Track file existence separately because stale manifest paths can block content-based inference.
