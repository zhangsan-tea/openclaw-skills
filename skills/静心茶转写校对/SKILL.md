---
name: 静心茶转写校对
description: 对静心茶练习记录 .md 做两轮处理——五层交叉互检（音转文硬伤、中文反推英文断句、正念语义校准、修辞保留、输出标记）与标点分段优化（按语义收句号、合并同语义行、超长段补拆、段落适配手机阅读）；含长文分块处理、保真率校验、说话人整体颠倒还原与 L1 残留机械补正。
read_when:
  - 用户要求校对静心茶转写记录
  - 用户要求纠正音转文错误、断句、标点
  - 用户要求调整练习记录的标点与分段
...
| 形态 | 特征 | 注意 |
|---|---|---|
| 会议转写 | `**Bommie**` / 口译交替 | >1000 行按 3 块分块处理 |
| Get 笔记·中英混接长行式 | 单行可达 1000 字符 | 按中英边界断行，勿整行保留 |
| Get 笔记·中文段落式 | 纯中文段落 | 无英文可反推，只做标点分段与明显错字 |
